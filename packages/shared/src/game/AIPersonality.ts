...
/** Save a custom preset. Overwrites if name already exists. */
export function saveCustomPreset(preset: CustomAIPreset): void {
  ...
  try {
    ...
    getStorage().setItem(CUSTOM_PRESETS_KEY, JSON.stringify(serialized));
  } catch {
...
export function loadCustomPresets(): CustomAIPreset[] {
  try {
    const raw = getStorage().getItem(CUSTOM_PRESETS_KEY);
    if (!raw) return [];
...
export function deleteCustomPreset(name: string): void {
  try {
    const existing = loadCustomPresets();
    const filtered = existing.filter((p) => p.name !== name);
    getStorage().setItem(CUSTOM_PRESETS_KEY, JSON.stringify(filtered.map(serializePreset)));
  } catch {
    // Storage unavailable — silently ignore
  }
}
